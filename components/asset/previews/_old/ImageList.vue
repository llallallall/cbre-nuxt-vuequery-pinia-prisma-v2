<template>
        <div class="image-slide w-full h-[500px] select-none">
                <img ref="slide"
                        :src="images.length < 1 ? '/images/placeholder.jpg' : images[images.length-1]?.fileUrl "
                ...
                <div class="small-previews w-full h-[100px] flex justify-center items-end gap-3 z-10 ">
                        <div v-for="preview in images.filter((el:any) => el.fileUrl && (el.fileUrl+'').trim().length > 0 )"
                                ...
                                <img :src="(preview?.fileUrl)"
                                        @click="setImageSrc(preview?.fileUrl as string)"
...
const currentImgUrl = ref(images[images.length-1]?.fileUrl)
