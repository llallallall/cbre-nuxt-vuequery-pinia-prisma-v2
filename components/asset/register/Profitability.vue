                        ...
                        flex flex-col gap-5">
                                ...
                                <ClientOnly>
                                        <VeeForm :validation-schema="validationSchema" @submit="handleSubmit" v-slot="{ meta }"
                                                class=" space-y-5">
                                                <VeeField name="grade" v-slot="{ field, meta }" v-model="profitability.grade">
                                                        <div class="relative w-full">
                                                                <input v-bind="field" type="text" :class="{
                                                                        ...
                                                                        autocomplete="one-time-code" placeholder="Grade" />
                                                                ...
                                                                <VeeErrorMessage name="grade" as="div"
                                                ...
                                                <VeeField name="effectiveRatio" v-slot="{ field, meta }"
                                                        v-model="profitability.effRatio">
                                                        <div class="relative w-full">
                                                                <input v-bind="field" type="text" :class="{
                                                                        'bg-cbre_primary_2/10': meta.valid && meta.touched,
                                                                        'bg-cbre_system_12/10': !meta.valid && meta.touched
                                                                ...
                                                                <VeeErrorMessage name="effectiveRatio" as="div"
                                                                        class="w-full absolute -bottom-[1.5em] left-[2.0em] bg-transparent text-cbre_system_12/60 text-xs flex items-end" />
                                                        </div>
                                                </VeeField>
...
const { debug, shrinkPreview, growPreview, profitability } = storeToRefs(usePropertyStore());
...
const validationSchema = toTypedSchema(object(
        {
                grade: string().required().max(2).label("Grade"),
                effective_ratio: string().nullable().label("Effective Ratio"),
        }
));
