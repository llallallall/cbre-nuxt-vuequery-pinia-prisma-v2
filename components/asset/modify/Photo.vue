...
const deleteFileData = async (response: any) => {
        ...
        let s3FileName
        if(response.uuid) {
                s3FileName = photoList.value.find((el : any) => el.uuid == response.uuid)?.fileKey
        ...
        try {
                ...
                const signedUrl = await $fetch('/api/upload/s3SignedUrl', {
                                method: 'delete',
                                        query: {
                                                key: s3FileName
                                        },
                ...
                const result = await useFetch(signedUrl.deleteUrl     , {
                        method: 'DELETE',
                        headers: {
                ...
                if (result.status.value === "success") {
                        ...
                        //console.log('deleted from aws s3')

                // 2) delete file on file list 
                if(response.uuid) {
                        let index = photoList.value.findIndex((el:any) => el.uuid == response.uuid)
                        if( index > -1) {
                                photoList.value = photoList.value.filter((el:any) => el.uuid !== response.uuid)
                                currentFileList.value = currentFileList.value.filter((el : any) => el.uuid !== response.uuid)
                ...
                // 4) TOAST                                        
                 createToast({
                                ...
                                //@ts-ignore
                                description: signedUrl.fileName
                                }, {
